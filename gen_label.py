...
ls_label_prompt = ['1_4', '2_1'] # prompts to gen label
...
class GenLabelAzureGPT:
  ...
  def gen_per_request(self, ls_base64_image, ls_pos, ls_neg, api_version='2024-02-01', deployment_name='gpt-4o-test'): ### api_version: 2023-12-01-preview, deployment_name=gpt-4v
      '''
    ...
    for n in range(self.max_retries):
      ...
      try:
        ...
        if n == self.max_retries-1:
          print(f'1: s{id} still encounters request rejection after {n+1} tries:', e)
          dict_ind_error[id] = temp_resp
      
      if '<REPORT>' in temp_repo or '<LABEL>' in temp_repo:
        # Check whether format error of label exists
        if self.prompt_label in ls_label_prompt:
          df_labels, dict_ind_error, is_retry = self.label_split(temp_repo, id, dict_ind_error, df_labels)
          if is_retry == 0:
            dict_ind_error.pop(id, None)
            break
          elif n == self.max_retries-1:
            temp_repo = ''
            print(f's{id} still encounters format error after {n+1} tries')
  ...
  def label_split(self, repo, id, dict_ind_error, df_labels):
    ...
    is_retry = 0
    if match:
        # Extract the matched content and convert it to a list of tuples using literal_eval for safety
        ...
        try:
          ...
          is_retry = 1
          dict_ind_error[id] = f'Wrong Format: {e}' # Syntax: "0""; Value: no label

    else:
        dict_ind_error[id] = 'Wrong Format'
        is_retry = 1
    
    return df_labels, dict_ind_error, is_retry
  ...
  def run(self):
    ...
    for i in tqdm(range(len(ls_id))):
      ...
      if n == self.max_retries-1:
        if '<LABEL>' not in repo_gen and '<REPORT>' not in repo_gen:
          if id not in dict_ind_error:
            print(f'2: s{id} still encounters request rejection after {n+1} tries: APIError')
            dict_ind_error[id] = 'APIError'
            continue

      if self.prompt_label in ls_label_prompt:
        df_labels, dict_ind_error, _ = self.label_split(repo_gen, id, dict_ind_error, df_labels)
